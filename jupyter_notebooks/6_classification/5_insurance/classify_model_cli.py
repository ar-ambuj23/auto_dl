...
def create_model(params):    
    ...
    if(np.isnan(acc)):
        print("{}) Testing set Accuracy: NaN".format(num),"\tAvailable Mem:",(mem.available/1024)/1024,"mb")
        num = num + 1
        return {'loss': np.inf, 'status': STATUS_OK, 'model': model}
    print("{}) Testing set Accuracy: {:7.2f}%".format(num,acc*100),"\tAvailable Mem:",(mem.available/1024)/1024,"mb")
    num = num + 1
...
if __name__ == '__main__':
    ...
    if(len(sys.argv) == 5):
        try:
            if(sys.argv[4] not in ['classify']):#,'predict','forecast']):
