...
def data(csv_name,label_col,test_arg,use_lime,features_subset):    
    ...
    for name, file in zip(['x_train', 'x_test', 'x_valid', 'y_train', 'y_test', 'y_valid'],[x_train, x_test, x_valid, y_train, y_test, y_valid]):
#         print(file)
        with open('{}.pkl'.format(name), 'wb') as output_file:
...
def update_config_db(csv_name,label_col,selected_cols,accuracy,model_type):
    try:
        deleteQuery(csv_name)
    except:
        pass
    values = (csv_name,model_type,str(selected_cols).replace("'",'"'),label_col,accuracy)
    insertQuery(values)
...
def save(csv_name,label_col,model,output_df,selected_cols,accuracy,model_type):
    ...
    update_config_db(csv_name=csv_name,label_col=label_col,selected_cols=selected_cols,accuracy=accuracy,model_type=model_type)
...
def get_best_cols_lime(intensity_df):
    ...
    intensity_df_trans.to_csv('classify_intensity_df.csv')
...
def driver(csv_name,label_col,test_array,model_type):
    ...
    if(save_model in ['Y','y']):
        if(accuracy_lime != -1):
            if(accuracy>accuracy_lime):
                save(csv_name=csv_name,label_col=label_col,model=best_model, output_df=output_df, selected_cols = list(training_dummy_columns), accuracy=accuracy,model_type=model_type)
                flush_memory()
            else:
                save(csv_name=csv_name,label_col=label_col,model=best_model_lime, output_df=output_df_lime, selected_cols = features_subset, accuracy=accuracy_lime,model_type=model_type)
                flush_memory()
        else:
            save(csv_name=csv_name,label_col=label_col,model=best_model, output_df=output_df, selected_cols = list(training_dummy_columns), accuracy=accuracy,model_type=model_type)
            flush_memory()
...
driver(csv_name='iris',label_col='class',test_array=[None],model_type='classify')        
