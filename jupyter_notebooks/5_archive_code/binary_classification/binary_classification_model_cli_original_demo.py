...
def display_results(model,output_df,label_col,test_arg_processed):    
    ...
    print("Precision on the test data: {:7.2f}%".format(prec*100))    
    print("#####################################################")
    for test_df in test_arg_processed:
        ...
        y_test_pred_temp = model.predict(test_df.loc[:, test_df.columns != label_col]).flatten()
        y_test_pred_temp = np.where(y_test_pred_temp>=0.5,1,0)
        prec_temp = precision_score(test_df[label_col],y_test_pred_temp)
        print("Accuracy on the given test csv: {:7.2f}%".format(acc_temp*100))
        print("Precision on the given test csv: {:7.2f}%".format(prec_temp*100))
        print("#####################################################")
    return acc, prec

# Save the model and update the config db
def update_config_db(csv_name,label_col,selected_cols,accuracy,precision,model_type):
    ...
    try:
        ...
        pass
    insert_query = "INSERT INTO config_table (model_name, model_type,list_of_features,target_column,accuracy,prec) VALUES (%s, %s, %s, %s, %s, %s)"
    values = (csv_name,model_type,str(selected_cols),label_col,accuracy,precision)
...
def save(csv_name,label_col,model,output_df,selected_cols,accuracy,precision,model_type):
    ...
    update_config_db(csv_name=csv_name,label_col=label_col,selected_cols=selected_cols,accuracy=accuracy,precision=precision,model_type=model_type)
...
def driver(csv_name,label_col,test_array,model_type):
    ...
    accuracy,precision = display_results(model=best_model, output_df=output_df, label_col=label_col, test_arg_processed=test_arg_processed)
    accuracy_lime = -1
    while(True):
    ...
    if(feature_selection in ['Y','y']):
        ...
        if(len(features_subset)!=0):
            ...
            accuracy_lime, precision_lime = display_results(model=best_model_lime, output_df=output_df_lime,label_col=label_col,test_arg_processed=test_arg_processed_lime)
        else:
            accuracy_lime = -1
    ...
    if(save_model in ['Y','y']):
        if(accuracy_lime != -1):
            if(accuracy>accuracy_lime):
                save(csv_name=csv_name,label_col=label_col,model=best_model, output_df=output_df, selected_cols = list(training_dummy_columns), accuracy=accuracy,precision=precision,model_type=model_type)
                flush_memory()
            else:
                save(csv_name=csv_name,label_col=label_col,model=best_model_lime, output_df=output_df_lime, selected_cols = features_subset, accuracy=accuracy_lime,precision=precision_lime,model_type=model_type)
                flush_memory()
        else:
            save(csv_name=csv_name,label_col=label_col,model=best_model, output_df=output_df, selected_cols = list(training_dummy_columns), accuracy=accuracy,precision=precision,model_type=model_type)
            flush_memory()
