...
def pre_process(df,label_col):
    ...
    if(len(class_names) > 20):
        print('The number of classes in the target column should be less than 20. \nExiting the script...')
        flush_memory()
...
def pre_process_test_csv(df,label_col,training_dummy_columns):
    ...
    dummy_Y_copy = pd.get_dummies(y,sparse=True)
    pre_processed_df_copy = pd.merge(df_copy,dummy_Y_copy,left_index=True,right_index=True)
    return pre_processed_df_copy
    
# Train Test Split
def split_train_test(df,test_size=0.2):    
    X = df[df.columns.difference(dummy_Y_cols)]
    y = df[dummy_Y_cols]
...
def get_class_weights(x_train,label_col):
    from sklearn.utils import class_weight
    class_weights = class_weight.compute_class_weight('balanced',np.unique(x_train[label_col]),x_train[label_col])
...
def data(csv_name,label_col,test_arg,use_lime,features_subset):    
    ...
    x_train, x_test, y_train, y_test = split_train_test(df=pre_processed_data)
    x_train, x_valid, y_train, y_valid = train_test_split(x_train, y_train, test_size=0.25, random_state=42)
    global class_weights
    class_weights = get_class_weights(x_train=x_train,label_col=label_col)
    x_train = x_train[x_train.columns.difference([label_col])]
    x_valid = x_valid[x_valid.columns.difference([label_col])]
    x_test = x_test[x_test.columns.difference([label_col])]
    test_arg_processed = []
    if(test_arg!=None):
...
def create_model(params):
    ...
    if(np.isnan(acc)):
        print("({}) Validation Accuracy: NaN".format(num),"|","Remaining Available Mem:",(mem.available/1024)/1024,"mb","|","Estimated time remaining:",int(h),"hours",int(m),"minutes",int(s),"seconds","\n")
        num = num + 1
        return {'loss': np.inf, 'status': STATUS_OK, 'params': params}
    print("({}) Validation Accuracy: {:7.2f} %".format(num,acc*100),"|","Remaining Available Mem:",(mem.available/1024)/1024,"mb","|","Estimated time remaining:",int(h),"hours",int(m),"minutes",int(s),"seconds","\n")
    num = num + 1
...
def get_best_model(csv_name,label_col,test_arg,use_lime=None,features_subset=None):
    ...
    pred_df = make_predictions(model=best_model_temp,scaled_feature_df=scaled_feature_df,label_df=label_df,label_col=label_col)
    output_df = pd.merge(input_df,pred_df,left_index=True,right_index=True)
...
# Make Predictions
def make_predictions(model,scaled_feature_df,label_df,label_col):    
    predictions = pd.DataFrame(model.predict(scaled_feature_df))
    predictions.index = label_df.index
    predictions.columns = label_df.columns
    predicted_col = reverse_dummies(predictions,label_col)
...
def display_results(model,output_df,label_col,test_arg_processed):    
    ...
    print("Accuracy on the test data: {:7.2f}%".format(acc*100))
    print("#####################################################")
    for test_df in test_arg_processed:
        loss_temp,acc_temp = model.evaluate(test_df.loc[:, test_df.columns != label_col],test_df[label_col],verbose=0)
        print("Accuracy on the given test csv: {:7.2f}%".format(acc_temp*100))
        print("#####################################################")
    return acc

# Save the model and update the config db
def update_config_db(csv_name,label_col,selected_cols,accuracy,model_type,source):
    try:
        deleteQuery(csv_name)
    except:
        pass
    values = (csv_name,model_type,str(selected_cols).replace("'",'"'),label_col,accuracy,source,unique_id_column)
    insertQuery(values)
...
def save(csv_name,label_col,model,output_df,selected_cols,accuracy,model_type,whether_duplicate):
    ...
    update_config_db(csv_name=csv_name,label_col=label_col,selected_cols=save_feature_list,accuracy=accuracy,model_type=model_type,source=source)
...
def imp_features_lime():
    ...
    explainer = lime.lime_tabular.LimeTabularExplainer(np.array(x_train), feature_names=selected_features, class_names=[], verbose=False, mode='classification')
...
def simulation_of_features(df,acc_val_ori):
    ...
    acc_v = get_results_per_simulation(best_params,feature_subset_selected)
    
    if(acc_v < acc_val_ori):
...
def get_best_cols_lime(intensity_df):
    ...
    loss_v,acc_v = best_model.evaluate(x_valid,y_valid,verbose=0)
    
    good_features, bad_features = simulation_of_features(intensity_df_trans,acc_v)
...
def driver(csv_name,label_col,test_array,model_type):
    ...
    accuracy = display_results(model=best_model, output_df=output_df, label_col=label_col, test_arg_processed=test_arg_processed)
    accuracy_lime = -1
    ...
    if(feature_selection in ['Y','y']):
        ...
        if(len(features_subset)!=0):
            ...
            accuracy_lime = display_results(model=best_model_lime, output_df=output_df_lime,label_col=label_col,test_arg_processed=test_arg_processed_lime)
        else:
            accuracy_lime = -1
    ...
    if(accuracy_lime == -1):
        ...
        print("Training Size: {} rows".format(x_train.shape[0]))
        print("Testing Size: {} rows".format(x_test.shape[0]))
        print("Accuracy on the test data: {:7.2f}%".format(accuracy*100)) 
        print("#####################################################")
        while(True):
        ...
        if(save_model in ['Y','y']):
            save(csv_name=csv_name,label_col=label_col, model=best_model, output_df=output_df, selected_cols = training_dummy_columns, accuracy=accuracy,model_type=model_type,whether_duplicate=whether_duplicate)
            flush_memory()
        else:
            print("Exiting the script...")
            flush_memory()
        ...
        print("Training Size: {} rows".format(x_train.shape[0]))
        print("Testing Size: {} rows".format(x_test.shape[0]))
        print("Accuracy on the test data: {:7.2f}%".format(accuracy_lime*100))
        print("#####################################################")
        while(True):
        ...
        if(save_model == '1'):
            ...
            save(csv_name=csv_name,label_col=label_col,model=best_model_lime, output_df=output_df_lime, selected_cols = features_subset, accuracy=accuracy_lime,model_type=model_type,whether_duplicate=whether_duplicate)
            flush_memory()
        else:
            print("Exiting the script...")
            flush_memory()
